...
namespace
{
  ...
  {
  public:
    explicit ControllerServiceImpl(const std::shared_ptr<grpc::Channel> &retrieval_channel)
    ...
    grpc::Status Search(grpc::ServerContext * /*context*/, const search::common::SearchQuery *request,
                        search::controller::SearchResponse *response) override
    {
  ...
  private:
    ...
    // route between.
    std::string Classify(const search::common::SearchQuery & /*query*/) { return "default"; }

    // Phase 1: a single passthrough call to Retrieval. Later phases
    // fan out to Retrieval + Cache + Sorter in parallel here instead.
    grpc::Status Orchestrate(const search::common::SearchQuery &query, const std::string & /*route*/,
...
int main()
{
  ...
  std::shared_ptr<grpc::Channel> channel =
      grpc::CreateChannel(retrieval_address, grpc::InsecureChannelCredentials());
